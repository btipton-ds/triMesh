...
class CSpatialSearchBase2D;

template <class SCALAR_TYPE, class INDEX_TYPE, int ENTRY_LIMIT>
using CSpatialSearchBase2DPtr = std::shared_ptr<CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>>;

template <class SCALAR_TYPE, class INDEX_TYPE, int ENTRY_LIMIT>
using CSpatialSearchBase2DConstPtr = std::shared_ptr<const CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>>;

template <class SCALAR_TYPE, class INDEX_TYPE, int ENTRY_LIMIT>
class CSpatialSearchBase2D : public std::enable_shared_from_this<CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>> {
public:
	using BOX_TYPE = CBoundingBox2D<SCALAR_TYPE>;
	using SpatialSearchBasePtr = CSpatialSearchBase2DPtr<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>;
	using SpatialSearchBaseConstPtr = CSpatialSearchBase2DConstPtr<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>;
	...
	CSpatialSearchBase2D(const BOX_TYPE& bbox = BOX_TYPE(), int axis = 0);
	virtual ~CSpatialSearchBase2D();
...
private:
	...
	void setSubContents(const BOX_TYPE& smallerBbox, const CSpatialSearchBase2D* pSrc, BoxTestType testType);
	...
	std::shared_ptr<CSpatialSearchBase2D> _pLeft, _pRight;
};

template <class SCALAR_TYPE, class INDEX_TYPE, int ENTRY_LIMIT>
inline size_t CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>::numInTree() const {
	return _numInTree;
}

template <class SCALAR_TYPE, class INDEX_TYPE, int ENTRY_LIMIT>
const typename CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>::BOX_TYPE& CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, ENTRY_LIMIT>::getBounds() const {
	return _bbox;
}

template <typename SCALAR_TYPE, typename INDEX_TYPE>
using CSpatialSearch2D = CSpatialSearchBase2D<SCALAR_TYPE, INDEX_TYPE, 25>;

template <typename SCALAR_TYPE>
using CSpatialSearch2DST = CSpatialSearch2D<SCALAR_TYPE, size_t>;

using CSpatialSearch2Dd = CSpatialSearch2DST<double>;
using CSpatialSearch2Df = CSpatialSearch2DST<float>;

//#include <tm_spatialSearch2D.hpp>
