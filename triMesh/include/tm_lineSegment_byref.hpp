...
template<class T>
bool LineSegment_byref<T>::intersectPlane(const Plane<T>& plane, bool includeEndPoints, RayHit<T>& hit, T tol) const
{
//	SEG_INTERSECT_PLANE
...
template<class T>
bool LineSegment_byref<T>::intersectPlane(const Plane_byref<T>& plane, bool includeEndPoints, RayHit<T>& hit, T tol) const
{
//	SEG_INTERSECT_PLANE
	if (plane.intersectRay(getRay(), hit, tol)) {
		auto v = hit.hitPt - _pt0;
...
template<class T>
inline bool LineSegment_byref<T>::intersectPlane(const POINT_TYPE* pts[3], bool includeEndPoints, RayHit<SCALAR_TYPE>& hit, SCALAR_TYPE tol) const
{
