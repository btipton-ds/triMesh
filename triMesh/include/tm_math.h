...
template<class T>
bool intersectTriTri(class Vector3<T> const* const* triPts0, class Vector3<T> const* const* triPts1, T tol = sameDistTol<T>());
...
template<class T>
bool BI_LERP_INV(const Vector3<T>& pt, const std::vector<Vector3<T>>& pts, T& t, T& u, T tol = sameDistTol<T>());
